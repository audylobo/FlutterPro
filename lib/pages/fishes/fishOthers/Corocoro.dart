...
void coro() => runApp(Corocoro());
var currentCar = pezList.fishs[0];

class Corocoro extends StatelessWidget{
...
class Coroco extends StatelessWidget{
...
class _PezCarouselState extends State<PezCarousel>{
  ...
  final List<Widget> child = _map<Widget>(imgList,(index, String asstName){
    return Container(
      child: Image.asset(  'assets/images/Corocoro.png', fit: BoxFit.fitWidth),
...
PezList pezList = PezList( fishs: [
  ...
  Peces1(nombreEspecie: "Corocoro", nombrePez: "Haemulon plumierii", pais:"Venezuela", imglist: [

    'assets/images/Corocoro.png',
  ], detalle:[

    {Icon(Icons.favorite, size: icoSize, color: Colors.pink):"EDAD MAX: 3 Años"},
    {Icon(FontAwesomeIcons.weightHanging, size: icoSize, color: Colors.blue):"Peso Max: 5000 gramos"},

  ],mediciones: [
    {Icon(Icons.control_point, size: icoSize, color: Colors.green):"Tamaño Maximo: 63 cm"},
    {Icon(Icons.remove_circle_outline, size:icoSize, color: Colors.red):"Tamaño Minimo: 30 cm"},

  ],estiloDeVida: [
    {Icon(FontAwesomeIcons.temperatureHigh, size:icoSize, color:Colors.red):{"Temperatura maxima(upto)": "17 "}},
   ...
   {Icon(FontAwesomeIcons.indent, size: icoSize, color: Colors.pink):{"Descripcion": "Normalmente tienen cinco rayas de color amarillo a marrón en la parte superior del cuerpo (no hay rayas en el vientre). 2 Hocico corto; ojos cerca de la boca."}}
  ]
  ),
]);
